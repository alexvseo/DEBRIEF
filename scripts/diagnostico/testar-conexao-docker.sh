...
if docker exec debrief-backend timeout 5 bash -c "echo > /dev/tcp/$DB_HOST/$DB_PORT" 2>/dev/null; then
    ...
    echo "   DATABASE_URL=postgresql://root:Mslestrategia.2025%40@host.docker.internal:5432/dbrief"
    echo ""
    echo "2️⃣  Usar IP do host ($HOST_IP):"
    echo "   DATABASE_URL=postgresql://root:Mslestrategia.2025%40@$HOST_IP:5432/dbrief"
    echo ""
