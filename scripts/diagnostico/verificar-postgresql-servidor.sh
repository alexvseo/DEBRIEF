...
print_info "7️⃣  Testando conexão com usuário 'postgres' e senha 'Mslestrategia.2025@'..."
...
if psql -h localhost -U postgres -d dbrief -c "SELECT 1;" > /dev/null 2>&1; then
    ...
    echo "sudo -u postgres psql -c \"ALTER USER postgres WITH PASSWORD 'Mslestrategia.2025@';\""
fi
unset PGPASSWORD
echo ""
